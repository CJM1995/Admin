...
if (!isset($_SESSION['admin_email'])) {
    ...
    if (isset($_GET['edit_rent_purchase_h'])) {
        $edit_id = $_GET['edit_rent_purchase_h'];
    ...
    if (isset($_POST['update'])) {
        ...
        $hp_rent_du = $_POST['duration'];
        $hp_pay_status = $_POST['pay_status'];

        if (empty($hp_inv_no)) {
            echo "<script>alert('Invoice No. cannot be empty!')</script>";
        } else {
            $update_hp = "update house_purchases set invoice_no='$hp_inv_no',p_date='$hp_pay_day',duration='$hp_rent_du',pay_status='$hp_pay_status' where hp_id='$hp_id'";
        }
